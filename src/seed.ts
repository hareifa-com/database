  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const [newPlayer] = await db.insert(players).values({
        ...
        dominant_foot: ['right', 'left', 'both'][Math.floor(Math.random() * 3)] as 'right' | 'left' | 'both',
        primary_position: ['GK', 'CB', 'LB', 'RB', 'CM', 'AM', 'LW', 'RW', 'ST'][Math.floor(Math.random() * 9)] as 'GK' | 'CB' | 'LB' | 'RB' | 'CM' | 'AM' | 'LW' | 'RW' | 'ST',
        secondary_position: Math.random() > 0.5 ? ['GK', 'CB', 'LB', 'RB', 'CM', 'AM', 'LW', 'RW', 'ST'][Math.floor(Math.random() * 9)] as 'GK' | 'CB' | 'LB' | 'RB' | 'CM' | 'AM' | 'LW' | 'RW' | 'ST' : null,
    ...
    for (const player of playersForNotes) {
      await db.insert(coachNotes).values({
        ...
        attendance: ['حضر', 'غائب', 'متأخر', 'مصاب'][Math.floor(Math.random() * 4)] as 'حضر' | 'غائب' | 'متأخر' | 'مصاب',
