...
export const evaluations = pgTable('evaluations', {
  ...
  evaluator_role: varchar('evaluator_role', { length: 20 }).notNull().$type<'مدرب' | 'كشاف' | 'متطوع'>(),
  
  /** Type of evaluation */
  evaluation_type: varchar('evaluation_type', { length: 20 }).notNull().$type<'مدرب' | 'كشاف' | 'متطوع'>(),
  ...
  overall_potential: varchar('overall_potential', { length: 1 }).notNull().$type<'A' | 'B' | 'C' | 'D'>(),
  ...
  weight: integer('weight').default(1).$type<1 | 2 | 3>(),
  
  /** Evaluation status in review process */
  status: varchar('status', { length: 20 }).default('قيد_المراجعة').$type<'معتمد' | 'قيد_المراجعة' | 'مرفوض'>(),
