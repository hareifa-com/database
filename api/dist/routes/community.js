...
import { communityReports, reputationScores, communityBadges, communityContributions, verificationRequests, communityActivities } from '@hareifa/database/schema';
...
router.get('/reports', async (req, res, next) => {
    try {
        ...
        const where = [];
        if (status)
            where.push(eq(communityReports.status, status));
        if (priority)
            where.push(eq(communityReports.priority, priority));
...
router.get('/contributions', async (req, res, next) => {
    try {
        ...
        const where = [];
        if (type)
            where.push(eq(communityContributions.contribution_type, type));
        if (status)
            where.push(eq(communityContributions.status, status));
...
router.get('/verification-requests', async (req, res, next) => {
    try {
        ...
        const where = [];
        if (status)
            where.push(eq(verificationRequests.status, status));
...
router.get('/activities', async (req, res, next) => {
    try {
        ...
        const where = [];
        if (user_id)
            where.push(eq(communityActivities.user_id, user_id));
