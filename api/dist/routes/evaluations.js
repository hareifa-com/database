...
router.get('/', async (req, res, next) => {
    try {
        const { page = 1, limit = 20, player_id, evaluator_id, status, min_score, max_score } = req.query;
        // Build where conditions
        const where = [];
        if (player_id) {
            where.push(eq(evaluations.player_id, player_id));
        }
        if (evaluator_id) {
            where.push(eq(evaluations.evaluator_id, evaluator_id));
        }
        if (status) {
            where.push(eq(evaluations.status, status));
        }
        if (min_score) {
            where.push(evaluations.overall_score >= parseFloat(min_score));
        }
        if (max_score) {
            where.push(evaluations.overall_score <= parseFloat(max_score));
        }
...
router.post('/', requirePermission(['evaluate_players']), async (req, res, next) => {
    try {
        ...
        const existingEvaluation = await db
            .select()
            .from(evaluations)
            .where(and(eq(evaluations.player_id, player_id), eq(evaluations.evaluator_id, req.user.id), eq(evaluations.status, 'approved')))
            .limit(1);
...
router.get('/player/:playerId', async (req, res, next) => {
    try {
        ...
        const playerEvaluations = await db
            ...
            .from(evaluations)
            .leftJoin(users, eq(evaluations.evaluator_id, users.id))
            .where(and(eq(evaluations.player_id, playerId), eq(evaluations.status, 'approved')))
            .orderBy(desc(evaluations.created_at))
            .limit(Number(limit))
            .offset((Number(page) - 1) * Number(limit));
        const totalCount = await db
...
router.get('/stats', requirePermission(['view_analytics']), async (req, res, next) => {
    try {
        ...
        const thisMonthEvaluations = await db
            .select({ count: count() })
            .from(evaluations)
            .where(and(eq(evaluations.status, 'approved'), evaluations.created_at >= thisMonth));
        res.json({
