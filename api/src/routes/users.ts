...
router.get('/', requireRole(['admin', 'regional_admin']), async (req, res, next) => {
  try {
    ...
    if (role) {
      where.push(eq(users.role, role as string));
    }
    
    if (governorate) {
      where.push(eq(users.governorate, governorate as string));
    }
...
router.get('/stats', requirePermission(['view_analytics']), async (req, res, next) => {
  try {
    ...
    const usersByGovernorate = await db
      ...
      .where(and(
        eq(users.is_active, true),
        eq(users.governorate, users.governorate)
      ))
