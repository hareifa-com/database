...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (type) {
      where.push(eq(notifications.type, type as string));
    }
    ...
    if (priority) {
      where.push(eq(notifications.priority, priority as string));
    }
