...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (governorate) {
      where.push(eq(players.governorate, governorate as string));
    }
    
    if (position) {
      where.push(eq(players.primary_position, position as string));
    }
    ...
    if (age_min || age_max) {
      ...
      const minBirthDate = age_max ? new Date(today.getFullYear() - parseInt(age_max as string), today.getMonth(), today.getDate()) : undefined;
      const maxBirthDate = age_min ? new Date(today.getFullYear() - parseInt(age_min as string), today.getMonth(), today.getDate()) : undefined;
      
      if (minBirthDate) where.push(eq(players.birth_date, minBirthDate));
      if (maxBirthDate) where.push(eq(players.birth_date, maxBirthDate));
    }
...
router.get('/search', async (req, res, next) => {
  try {
    ...
    if (dominant_foot) {
      where.push(eq(players.dominant_foot, dominant_foot as string));
    }
    ...
    // Additional filters
    if (height_min) {
      where.push(eq(players.height_cm, parseInt(height_min as string)));
    }
    
    if (height_max) {
      where.push(eq(players.height_cm, parseInt(height_max as string)));
    }
